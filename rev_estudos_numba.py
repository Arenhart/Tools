...
def covariance_3d(arr):
    ...
    for ax in (0,1,2):
        for i in range(1, arr.shape[ax] - 1):
            ...
            limit_covariance = get_limit_covariance(arr[left_slice], arr[right_slice])
            if limit_covariance == 0:
                corr[ax][i] = 1
            else:
...
def covariance(arr):
    '''
    Correlation for n-dimensional scalar arrays
    '''

    ndim = arr.ndim
    corr = np.zeros((ndim, max(arr.shape)-1))
    corr[:,0] = 1

    for ax in range(ndim):
        for i in range(1, arr.shape[ax] - 1):
            #null_index = slice( *(None,) * ndim )
            null_index = slice( None, None, None )
            left_index = slice(i, None, 1)
            right_index = slice(0, -i, 1)
            left_slice = [null_index,] * ndim
            left_slice[ax] = left_index
            left_slice = tuple(left_slice)
            right_slice = [null_index,] * ndim
            right_slice[ax] = right_index
            right_slice = tuple(right_slice)
            limit_covariance = get_limit_covariance(arr[left_slice], arr[right_slice])
            if limit_covariance == 0:
...
def get_limit_covariance(arr1, arr2):
    ...
    for z in prange(z_max):
        for y in range(y_max):
            for x in range(x_max):
                ...
                _inscribe_spheres(sub_out, b_value, b.shape)
...
@njit(parallel = False)
def _inscribe_spheres(img, values, shape):
...
def maxi_balls_2d(img):
    calculated_circles = Dict.empty(key_type = int64, value_type = uint32[:,::1],)
    image_edt = ndimage.morphology.distance_transform_edt(img).astype('uint32')
    return _maxi_balls_2d(img, image_edt, calculated_circles)


#distances = ndimage.morphology.distance_transform_edt(img)#.astype('uint16')
@jit(nopython=True, parallel = False)
def _maxi_balls_2d(img, image_edt, calculated_circles):
    ...
    for y in prange(y_max):
        for x in range(x_max):
            ...
            if radius <= 0.5: continue
            point = np.array((x, y))
            b_size = np.int64((radius)*2 + 1)

            for key in calculated_circles.keys():
                if key == b_size: break
            else:
                new_circle = create_circle(b_size)
                if new_circle.shape[0]%2 == 0: new_circle[0:-1,0:-1].copy()
                calculated_circles[b_size] = new_circle
            b = calculated_circles[b_size]
            ...
            sub_out = out[lower_bounds[0]:upper_bounds[0], lower_bounds[1]: upper_bounds[1]]
            sub_img = img[lower_bounds[0]:upper_bounds[0], lower_bounds[1]: upper_bounds[1]]
            b_value = b * radius
            b_value = b_value * sub_img
            _inscribe_spheres_2d(sub_out, b_value, b.shape)
...
@njit(parallel = False)
def _inscribe_spheres_2d(img, values, shape):

    for x in prange(shape[0]):
        for y in range(shape[1]):
            if values[x, y] > img[x, y]:
                img[x, y] = values[x, y]
...
def create_circle(diameter):
    ...
    ind = np.zeros((2,diameter, diameter))
    for i in range(1, diameter):
        ind[0,i,:] = i
        ind[1,:,i] = i
    ind -= (diameter - 1) / 2
    circle = np.sqrt(ind[0, :, :]**2 + ind[1, :, :]**2)
    radius = (diameter-1)/2
    circle = (circle <= radius).astype(np.uint32)
    return circle
...
get_limit_covariance(dol_p)
get_limit_covariance(dol_n)
get_limit_covariance(dol_np)
